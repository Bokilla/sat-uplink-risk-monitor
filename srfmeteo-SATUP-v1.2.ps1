...
function Get-PredictedLinkHealth {
    ...
    if ($MarginTrend -eq 'Nicht genügend Historie' -or $BeaconTrend -eq 'Nicht genügend Historie') { return 'n.v.' }
...
function Get-SwitchoverRecommendation {
    ...
    if ("$ZurichOverallRisk" -eq 'n.v.' -or "$ComanoOverallRisk" -eq 'n.v.' -or "$PredictedFailureProbability" -eq 'n.v.' -or "$FutureRisk" -eq 'n.v.' -or ($ActiveSite -ne 'Zürich' -and $ActiveSite -ne 'Comano')) { return 'Keine Aktion erforderlich' }
    $difference = Get-UplinkSiteComparison -ZurichOverallRisk $ZurichOverallRisk -ComanoOverallRisk $ComanoOverallRisk
...
function Show-Weather {
    ...
    try {
        ...
        if (
            ...
            $forecastRiskValues = @($firstLocation.Forecasts | ForEach-Object { Get-FutureWeatherRiskScore -CurrentLinkHealth $zurichLinkHealth -Forecast $_ })
            $forecastRiskValues += @($secondLocation.Forecasts | ForEach-Object { Get-FutureWeatherRiskScore -CurrentLinkHealth $luganoLinkHealth -Forecast $_ })
            $futureRiskSignal = @($forecastRiskValues | Where-Object { "$($_)" -ne 'n.v.' } | Measure-Object -Maximum).Maximum
            if ($null -eq $futureRiskSignal) { $futureRiskSignal = 'n.v.' }
            ...
            $predictedFailureText = if ("$activePredictedFailure" -eq 'n.v.') { 'n.v.' } else { Get-FailureProbabilityText -FailureProbability $activePredictedFailure }
            ...
            Write-Host "================================================" -ForegroundColor DarkGray
            Write-Host "Link Health Prognose (1h)" -ForegroundColor Cyan
            Write-Host ("Zürich: {0} %" -f $zurichPredictedLinkHealth)
            Write-Host ("Comano: {0} %" -f $luganoPredictedLinkHealth)
            Write-Host ""
            Write-Host "Future Risk" -ForegroundColor Cyan
            $futureLabels = @('+3h', '+6h', '+9h')
            $forecastCount = @($firstLocation.Forecasts).Count
            if ($forecastCount -gt @($secondLocation.Forecasts).Count) { $forecastCount = @($secondLocation.Forecasts).Count }
            if ($forecastCount -gt 3) { $forecastCount = 3 }
            for ($futureIndex = 0; $futureIndex -lt $forecastCount; $futureIndex++) {
                $zurichFutureRisk = Get-FutureWeatherRiskScore -CurrentLinkHealth $zurichLinkHealth -Forecast $firstLocation.Forecasts[$futureIndex]
                $comanoFutureRisk = Get-FutureWeatherRiskScore -CurrentLinkHealth $luganoLinkHealth -Forecast $secondLocation.Forecasts[$futureIndex]
                Write-Host ("{0,-5} Zürich: {1}% ({2}) | Comano: {3}% ({4})" -f $futureLabels[$futureIndex], $zurichFutureRisk, (Get-RiskText -RiskScore $zurichFutureRisk), $comanoFutureRisk, (Get-RiskText -RiskScore $comanoFutureRisk))
            }
            Write-Host ""
            Write-Host "Ausfallwahrscheinlichkeit" -ForegroundColor Cyan
            Write-Host ("Aktuell: {0} % ({1})" -f $failureProbability, $failureText)
            Write-Host ("In 1h:   {0} % ({1})" -f $activePredictedFailure, $predictedFailureText)
            Write-Host ""
