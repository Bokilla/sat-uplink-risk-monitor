...
function Get-RfTrend {
    ...
    if ($values.Count -lt 2) { return 'Stabil' }
    ...
    $threshold = if ($Metric -eq 'Margin') { 0.5 } else { 1.0 }
    $strongThreshold = if ($Metric -eq 'Margin') { 1.0 } else { 3.0 }

    if ($delta -le -$strongThreshold) { return 'Stark fallend' }
    if ($delta -le -$threshold) { return 'Fallend' }
    if ($delta -ge $strongThreshold) { return 'Steigend' }
    if ($delta -ge $threshold) { return 'Steigend' }
    return 'Stabil'
}
...
function Get-SwitchoverRecommendation {
    ...
    if ("$ZurichOverallRisk" -eq 'n.v.' -or "$ComanoOverallRisk" -eq 'n.v.' -or ($ActiveSite -ne 'Zürich' -and $ActiveSite -ne 'Comano')) { return 'Keine Aktion erforderlich' }
    $difference = Get-UplinkSiteComparison -ZurichOverallRisk $ZurichOverallRisk -ComanoOverallRisk $ComanoOverallRisk
    ...
    $negativeTrend = $MarginTrend -in @('Fallend', 'Stark fallend') -or $BeaconTrend -in @('Fallend', 'Stark fallend')

    if ($difference -lt 10 -or $activeRisk -le $otherRisk -or -not $negativeTrend) { return 'Keine Aktion erforderlich' }
    if ($difference -ge 30) { return 'DRINGEND EMPFOHLEN' }
    if ($difference -ge 20) { return 'Umschaltung empfohlen' }
    return 'Umschaltung prüfen'
}
...
function Get-FailureProbability {
    ...
    $trendAdjustments = @{ 'Steigend' = -5; 'Stabil' = 0; 'Fallend' = 10; 'Stark fallend' = 20 }
...
function Show-Weather {
    ...
    try {
        ...
        Write-Host ("{0,-34} | {1,-28} | {2,-28}" -f "Weather Bewertung", (Get-WeatherRiskText -WeatherRiskScore $firstWeatherRisk), (Get-WeatherRiskText -WeatherRiskScore $secondWeatherRisk))
        ...
        if (
            ...
            Write-Host ("{0,-24} {1,-10} | {2,-10}" -f "Aktiv", $zurichActiveDisplay, $luganoActiveDisplay)
            ...
            Write-Host ("{0,-24} | {1,-10} | {2,-10}" -f "Bewertung", $zurichLinkHealthText, $luganoLinkHealthText)
            ...
            Write-Host ("{0,-24} | {1,-10} | {2,-10}" -f "Bewertung", $zurichOverallRiskText, $luganoOverallRiskText)
            ...
            Write-Host ("{0,-24} {1}" -f "Bewertung:", $activeOverallRiskText) -ForegroundColor White
            ...
            $recommendation = Get-SwitchoverRecommendation -ActiveSite $activeLocation -ZurichOverallRisk $zurichOverallRisk -ComanoOverallRisk $luganoOverallRisk -SiteAdvantage $siteAdvantage -MarginTrend $activeMarginTrend -BeaconTrend $activeBeaconTrend
            ...
            Write-Host ("{0,-18} {1}" -f "Zürich Margin:", $zurichMarginTrend)
            Write-Host ("{0,-18} {1}" -f "Zürich Beacon:", $zurichBeaconTrend)
            Write-Host ("{0,-18} {1}" -f "Comano Margin:", $comanoMarginTrend)
            Write-Host ("{0,-18} {1}" -f "Comano Beacon:", $comanoBeaconTrend)
            Write-Host ""
            Write-Host "Ausfallwahrscheinlichkeit" -ForegroundColor Cyan
            Write-Host ("Aktiver Uplink: {0} % ({1})" -f $failureProbability, $failureText)
            Write-Host ""
