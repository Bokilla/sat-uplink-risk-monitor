# Konfiguration:
# - site-config.json
# - api-credentials/clientId.txt
# - api-credentials/clientSecret.txt
#
...
function Get-SiteConfiguration {
    ...
    if (-not (Test-Path -Path $configPath)) {
        throw "site-config.json nicht gefunden: $configPath"
    }
    ...
    try {
        ...
        throw "site-config.json ungültig: $($_.Exception.Message)"
    }

    if ($null -eq $config) {
        throw 'site-config.json ungültig: Konfiguration ist leer.'
    }
...
try {
    $script:Config = Get-SiteConfiguration
}
catch {
    Write-Host "Konfiguration fehlt." -ForegroundColor Red
...
function Get-GrafanaPrometheusValue {
    ...
    if ([string]::IsNullOrWhiteSpace($GrafanaUrl) -or [string]::IsNullOrWhiteSpace($DatasourceUid) -or [string]::IsNullOrWhiteSpace($Query)) {
        return "n.v."
    }
...
function Test-GrafanaConnection {
    ...
    Write-Host "Grafana URL: $($script:Config.Grafana.Url)"
    Write-Host "Datasource UID: $($script:Config.Grafana.DatasourceUid)"

    $queries = @("up", $script:Config.Sites.Zurich.Monitoring.Active.Metric)
    $lastError = "Keine Daten für die Testqueries erhalten."

    foreach ($query in $queries) {
        try {
            $baseUrl = $script:Config.Grafana.Url.TrimEnd('/')
            ...
            $uri = "$baseUrl/api/datasources/proxy/uid/$($script:Config.Grafana.DatasourceUid)/api/v1/query?query=$encodedQuery"
            $response = Invoke-RestMethod -Uri $uri -Method Get -ErrorAction Stop
...
function Get-UplinkActiveState {
    ...
    $siteConfig = $script:Config.Sites.$SiteName
    if ($null -eq $siteConfig -or $null -eq $siteConfig.Monitoring.Active) { return "n.v." }
    if ("$RawValue" -eq "$($siteConfig.Monitoring.Active.ActiveValue)") { return "Aktiv" }
    if ("$RawValue" -eq "$($siteConfig.Monitoring.Active.InactiveValue)") { return "Inaktiv" }
...
function Get-UplinkRfMetrics {
    param(
        [string]$GrafanaUrl = $script:Config.Grafana.Url,
        [string]$DatasourceUid = $script:Config.Grafana.DatasourceUid
    )
    ...
    return @{
        Zurich = @{
            Active = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query $script:Config.Sites.Zurich.Monitoring.Active.Metric
            Path = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query $script:Config.Sites.Zurich.Monitoring.Path.Metric
            IRDMargin = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query ("{0} / {1}" -f $script:Config.Sites.Zurich.Monitoring.IRDMargin.Metric, $script:Config.Sites.Zurich.Monitoring.IRDMargin.ScaleFactor)
            BeaconLevel = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query ("{0} / {1}" -f $script:Config.Sites.Zurich.Monitoring.BeaconLevel.Metric, $script:Config.Sites.Zurich.Monitoring.BeaconLevel.ScaleFactor)
            Azimuth = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query ("{0} / {1}" -f $script:Config.Sites.Zurich.Monitoring.Azimuth.Metric, $script:Config.Sites.Zurich.Monitoring.Azimuth.ScaleFactor)
            Elevation = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query ("{0} / {1}" -f $script:Config.Sites.Zurich.Monitoring.Elevation.Metric, $script:Config.Sites.Zurich.Monitoring.Elevation.ScaleFactor)
        }
        Lugano = @{
            Active = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query $script:Config.Sites.Comano.Monitoring.Active.Metric
            Path = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query $script:Config.Sites.Comano.Monitoring.Path.Metric
            IRDMargin = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query ("{0} / {1}" -f $script:Config.Sites.Comano.Monitoring.IRDMargin.Metric, $script:Config.Sites.Comano.Monitoring.IRDMargin.ScaleFactor)
            BeaconLevel = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query ("{0} / {1}" -f $script:Config.Sites.Comano.Monitoring.BeaconLevel.Metric, $script:Config.Sites.Comano.Monitoring.BeaconLevel.ScaleFactor)
            Azimuth = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query ("{0} / {1}" -f $script:Config.Sites.Comano.Monitoring.Azimuth.Metric, $script:Config.Sites.Comano.Monitoring.Azimuth.ScaleFactor)
            Elevation = Get-GrafanaPrometheusValue -GrafanaUrl $GrafanaUrl -DatasourceUid $DatasourceUid -Query ("{0} / {1}" -f $script:Config.Sites.Comano.Monitoring.Elevation.Metric, $script:Config.Sites.Comano.Monitoring.Elevation.ScaleFactor)
        }
    }
}
...
function Show-OperationalView {
    ...
    try {
        ...
        $locations = @(
            @{ Name = $script:Config.Sites.Zurich.DisplayName; Latitude = $script:Config.Sites.Zurich.Location.Latitude; Longitude = $script:Config.Sites.Zurich.Location.Longitude }
            @{ Name = $script:Config.Sites.Comano.DisplayName; Latitude = $script:Config.Sites.Comano.Location.Latitude; Longitude = $script:Config.Sites.Comano.Location.Longitude }
        )
        $weatherData = @()
        ...
        $rfMetrics = Get-UplinkRfMetrics -GrafanaUrl $script:Config.Grafana.Url -DatasourceUid $script:Config.Grafana.DatasourceUid
...
function Show-Weather {
    ...
    try {
        ...
        if (
            ...
            $zurichPathConfig = $script:Config.Sites.Zurich.Monitoring.Path
            $comanoPathConfig = $script:Config.Sites.Comano.Monitoring.Path
            $zurichPath = if ("$($rfMetrics.Zurich.Path)" -eq "$($zurichPathConfig.PathAValue)") { "Pfad A" } elseif ("$($rfMetrics.Zurich.Path)" -eq "$($zurichPathConfig.PathBValue)") { "Pfad B" } else { "n.v." }
            $luganoPath = if ("$($rfMetrics.Lugano.Path)" -eq "$($comanoPathConfig.PathAValue)") { "Pfad A" } elseif ("$($rfMetrics.Lugano.Path)" -eq "$($comanoPathConfig.PathBValue)") { "Pfad B" } else { "n.v." }
        ...
        Write-Host ("{0,-24} {1}" -f "Satellit:", $script:Config.Satellite.Name) -ForegroundColor White
        Write-Host ("{0,-24} {1}" -f "Transponder:", $script:Config.Satellite.Transponder) -ForegroundColor White
        Write-Host ("{0,-24} {1}" -f "Uplink-Frequenz:", ("{0} MHz" -f $script:Config.Satellite.UplinkFrequencyMHz)) -ForegroundColor White
        Write-Host ""
        Write-Host "Zürich:" -ForegroundColor Cyan
        Write-Host ("{0,-24} {1}" -f "Antennendurchmesser:", ("{0} m" -f $script:Config.Sites.Zurich.Infrastructure.AntennaDiameterM)) -ForegroundColor White
        Write-Host ("{0,-24} {1}" -f "TX Power:", ("{0} dBm" -f $script:Config.Sites.Zurich.Infrastructure.TxPowerDbm)) -ForegroundColor White
        Write-Host ""
        Write-Host "Comano:" -ForegroundColor Cyan
        Write-Host ("{0,-24} {1}" -f "Antennendurchmesser:", ("{0} m" -f $script:Config.Sites.Comano.Infrastructure.AntennaDiameterM)) -ForegroundColor White
...
do {
    ...
    switch ($choice.ToUpper()) {
        ...
        "7" {
            Test-RfMetricQuery -Query $script:Config.Sites.Zurich.Monitoring.Active.Metric
            Test-RfMetricQuery -Query $script:Config.Sites.Zurich.Monitoring.IRDMargin.Metric
            Test-RfMetricQuery -Query $script:Config.Sites.Zurich.Monitoring.BeaconLevel.Metric
        }
